...
class GenericLLMProvider:
    ...
    def from_provider(cls, provider: str, chat_log: str | None = None, verbose: bool = True, **kwargs: Any):
        if provider == "openai":
            ...
            if "model" in kwargs:
                kwargs = {"azure_deployment": kwargs.get("model"), **kwargs}
    ...
    async def get_chat_response(self, messages, stream, websocket=None, **kwargs):
        if not stream:
            ...
            content = _join_content(content)
            if self.chat_logger:
                await self.chat_logger.log_request(messages, content)
            return content
        ...
        async for chunk in self.llm.astream(messages, **kwargs):
            ...
            if text:
                ...
                if websocket:
                    await websocket.send_json({"type": "stream", "output": text})
        if self.verbose and response:
            print(Style.RESET_ALL)
        final_response = "".join(response)
        if self.chat_logger:
            await self.chat_logger.log_request(messages, final_response)
