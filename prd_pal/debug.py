"""Quick debug script for prd_pal agents.

Usage
-----
# Test parser only
python -m prd_pal.debug --agent parser

# Test reviewer only (uses parser output from a previous run, or built-in mock)
python -m prd_pal.debug --agent reviewer

# Test full pipeline: parser → reviewer
python -m prd_pal.debug --agent all

# Use your own requirement document
python -m prd_pal.debug --agent all --input docs/prd.md

# Enable verbose logging
python -m prd_pal.debug --agent all -v
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Debug prd_pal agents")
    p.add_argument(
        "--agent",
