...
技术要点  
- 编排入口与模式分流：`prd_pal/workflow.py`
- gating / requirement normalization：`prd_pal/review/gating.py`、`prd_pal/review/normalizer.py`
- 多 reviewer 并行与聚合：`prd_pal/review/parallel_review_manager.py`、`prd_pal/review/aggregator.py`
- 运行与落盘：`prd_pal/run_review.py`、`prd_pal/service/review_service.py`
- 服务入口与结果读取：`prd_pal/server/app.py`、`prd_pal/mcp_server/server.py`
- 观测与质量：`prd_pal/utils/trace.py`、`eval/run_eval.py`、`tests/`
...
技术要点  
- 节点职责拆分：`prd_pal/agents/*.py`
- 状态驱动编排：`prd_pal/workflow.py`、`prd_pal/state.py`
...
- F3. Clarification loop 和 reviewer 冲突仲裁有什么区别？
  ...
  技术要点  
  `prd_pal/workflow.py` 负责 `routing_reason` 与 `revision_round`；`prd_pal/review/aggregator.py`、`prd_pal/review/reviewer_agents/delivery_reviewer.py` 负责 conflict detection、resolution 和 `needs_human`。
...
技术要点  
- 统一调用入口：`prd_pal/utils/llm_structured_call.py`
- schema 定义与校验：`prd_pal/schemas/*.py`
...
技术要点  
- 本地检索工具：`prd_pal/tools/risk_catalog_search.py`
- Skill 封装：`prd_pal/skills/risk_catalog.py`
- 子流接入：`prd_pal/subflows/risk_analysis.py`
...
技术要点  
- 技能注册：`prd_pal/skills/registry.py`
- 技能执行：`prd_pal/skills/executor.py`
- 风险检索 skill：`prd_pal/skills/risk_catalog.py`
- 技能消费方：`prd_pal/subflows/risk_analysis.py`
...
技术要点  
- TTL cache：`prd_pal/skills/executor.py`
...
技术要点  
- 子流实现：`prd_pal/subflows/risk_analysis.py`
- 主图接入：`prd_pal/workflow.py`
...
技术要点  
- 并行图：`prd_pal/workflow.py`
- merge reducer：`prd_pal/state.py`
- runtime metrics：`prd_pal/metrics/runtime.py`
...
技术要点  
- gating 规则与阈值：`prd_pal/review/gating.py`
- 结构化信号抽取：`prd_pal/review/normalizer.py`
- 主图中的模式切换：`prd_pal/workflow.py`
...
技术要点  
- reviewer 选择与超时容错：`prd_pal/review/parallel_review_manager.py`
- reviewer 视角切片：`prd_pal/review/normalizer.py`
- 四个 reviewer 实现：`prd_pal/review/reviewer_agents/*.py`
...
技术要点  
- 冲突检测与聚合：`prd_pal/review/aggregator.py`
- 仲裁规则：`prd_pal/review/reviewer_agents/delivery_reviewer.py`
- report 中的冲突与人工补审标记：`prd_pal/review/aggregator.py`
...
技术要点  
- FastAPI：`prd_pal/server/app.py`
- MCP：`prd_pal/mcp_server/server.py`
- 共享执行层：`prd_pal/service/review_service.py`
- MCP 报告读取：`prd_pal/service/report_service.py`
